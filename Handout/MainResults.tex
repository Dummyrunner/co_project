...
\begin{remark}\label{re:newtonStop}
	...
	For theoretical reasoining, or if $ \grad^2 f(x)^{-1} $ can be used explicitely, one can also use the decreasement of $ \lambda^2 = \grad \fx \trp \grad^2 f(x)^{-1} \grad f(x)$ (so called newton-decrement) under a certain tolerance.
\end{remark}
...
\end{align}
holds. Note that  the Lagrangian only depends on $ (x,\nuhat) $, since there are no explicit inequality constraints involved. We keep in mind, that $ \xoptt $ minim
