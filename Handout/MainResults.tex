...
\todo KKT cond.
Stacked in one vector to apply newton on, this yields
\begin{align}
\begin{split}
	 F(x) :&= \rmu = \vectorthree{\rdual}{\rcent}{\rpri}\\ &= \vectorthree{\grad \fnull(x) + \jac \fx \trp \lambda + \Aeq \trp \nu}{-\diag(\lambda)\fx - \mu \ones}{\Aeq x - \beq}
\end{split}
\end{align}
